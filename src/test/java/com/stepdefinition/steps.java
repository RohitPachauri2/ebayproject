...
public class steps {
	...
	public void user_is_on_home_page() {
		Assert.assertTrue(driver.getCurrentUrl().equals("https://www.ebay.com/"));
	}

	@When("user clicks o laptops section and enters lenovo in search box")
	public void user_clicks_o_laptops_section_and_enters_lenovo_in_search_box() {
		Actions action = new Actions(driver);
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(5));
		wait.until(ExpectedConditions
				.visibilityOf(driver.findElement(By.xpath("//a[@class='vl-popular-destinations-evo__link']//div[1]"))));
		WebElement laptops = driver.findElement(By.cssSelector("a.vl-popular-destinations-evo__link>div:first-child"));
		action.scrollToElement(laptops);
		laptops.click();

		wait.until(ExpectedConditions.visibilityOf(driver.findElement(By.id("gh-ac"))));
		WebElement sbtn = driver.findElement(By.id("gh-ac"));
		sbtn.sendKeys("Lenovo gaming" + Keys.ENTER);
	}

	@Then("gets all the lenovo laptops")
	public void gets_all_the_lenovo_laptops() throws InterruptedException {
		List<WebElement> list = driver
				.findElements(By.cssSelector("a.s-card__link>div>span[class='su-styled-text primary default']"));
		for (WebElement a : list) {
			System.out.println(a.getText());
		}
		Thread.sleep(2000);
	...
	public void user_hovers_on_fashion_and_clicks_on_deals() throws InterruptedException {
		Actions action=new Actions(driver);
		...
		wait.until(ExpectedConditions.visibilityOf(driver.findElement(By.linkText("Deals"))));
	...
	public void navigated_to_fashion_deals_page() {
		Assert.assertTrue(driver.getCurrentUrl().equals("https://www.ebay.com/globaldeals/fashion"));
	}
}
